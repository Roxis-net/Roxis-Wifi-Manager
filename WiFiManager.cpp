...
void WiFiManager::setupConfigPortal()
{
 ...
 /* Serial.print("^Ip Adresi=");
  ...
  server->on(String(F("/")).c_str(), std::bind(&WiFiManager::handleWifi, this, true));
...
void WiFiManager::handleRoot()
{
  ...
  String page = FPSTR("<html><head><meta charset=\"UTF-8\"><meta http-equiv=\"refresh\" content=\"time=0; URL=http://192.168.4.1/wifi\" /></head><body>Yönlendiriliyor.</body></html>");
...
void WiFiManager::handleWifi(boolean scan)
{
  ...
  String page = FPSTR(HTTP_HEADER);
  page.replace("{v}", "RX Lazer Kurulum");
  page += FPSTR(HTTP_SCRIPT);
  page += FPSTR(HTTP_STYLE);
  page += _customHeadElement;
  page += FPSTR(HTTP_HEADER_END);
